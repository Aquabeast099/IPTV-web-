    ...
    <div style={{ minHeight: "100vh", background: "#0a0e1a", color: "#e0e8f0", fontFamily: "'Courier New', monospace", display: "flex", flexDirection: "column" }}>
                        ...
                        >
                          ...
                          <td style={{ ...tdStyle, fontWeight: 600, color: "#e0e8f0", maxWidth: 200 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              {ch.logo && <img src={ch.logo} style={{ width: 20, height: 14, objectFit: "contain" }} onError={e => e.target.style.display = "none"} />}
                              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ch.name}</span>
                            </div>
                          </td>
                          <td style={{ ...tdStyle, color: "#ffaa00", fontSize: 11 }}>{ch.group}</td>
                          <td style={{ ...tdStyle, color: "rgba(255,255,255,0.4)", fontSize: 11 }}>{ch.tvgId}</td>
                          <td style={{ ...tdStyle, color: "rgba(0,200,255,0.5)", fontSize: 10, maxWidth: 200 }}>
                            <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ch.url}</div>
                          </td>
                          <td style={tdStyle}>
                            <button onClick={() => setEditingChannel({ ...ch })} style={{ background: "transparent", border: "1px solid rgba(0,200,255,0.3)", color: "#00c8ff", padding: "3px 8px", cursor: "pointer", fontSize: 11, fontFamily: "inherit" }}>✏</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {tab === "epg" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
              <div style={{ display: "flex", gap: 12, marginBottom: 20, alignItems: "center" }}>
                <input ref={fileInputXML} type="file" accept=".xml,.xmltv" onChange={handleXMLTVFile} style={{ display: "none" }} />
                <button onClick={() => fileInputXML.current.click()} style={btnStyle("#00c8ff")}>📂 Cargar XMLTV</button>
                {epgData && <input value={epgFilter} onChange={e => setEpgFilter(e.target.value)} placeholder="Buscar canal EPG..." style={{ ...inputStyle, width: 220 }} />}
                {epgData && <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>{epgData.channels.length} canales · {epgData.programmes.length} programas</span>}
              </div>
              {!epgData ? (
                <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(255,255,255,0.2)" }}>
                  <div style={{ fontSize: 40, marginBottom: 16 }}>📅</div>
                  <div>Carga un archivo XMLTV para ver la guía electrónica de programación</div>
                  <div style={{ fontSize: 12, marginTop: 8, opacity: 0.6 }}>Formatos soportados: .xml, .xmltv</div>
                </div>
              ) : (
                <>
                  <div style={{ color: "rgba(0,200,255,0.6)", fontSize: 10, letterSpacing: 2, marginBottom: 12 }}>CANALES EPG</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8, marginBottom: 24 }}>
                    {epgData.channels.filter(c => !epgFilter || c.id.toLowerCase().includes(epgFilter.toLowerCase()) || c.name.toLowerCase().includes(epgFilter.toLowerCase())).slice(0, 50).map(c => (
                      <div key={c.id} style={{ background: "rgba(0,200,255,0.05)", border: "1px solid rgba(0,200,255,0.1)", borderRadius: 2, padding: "10px 12px" }}>
                        <div style={{ color: "#00c8ff", fontSize: 12, fontWeight: 700 }}>{c.name}</div>
                        <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10 }}>{c.id}</div>
                        <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 10, marginTop: 4 }}>
                          {epgData.programmes.filter(p => p.channel === c.id).length} programas
                        </div>
                      </div>
                    ))}
                  </div>
                  <div style={{ color: "rgba(0,200,255,0.6)", fontSize: 10, letterSpacing: 2, marginBottom: 12 }}>PROGRAMACIÓN (últimos 20)</div>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {epgData.programmes.slice(0, 20).map((p, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                          <td style={{ ...tdStyle, color: "#ffaa00", fontSize: 11 }}>{p.channel}</td>
                          <td style={{ ...tdStyle, fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{p.start?.substring(0, 12)}</td>
                          <td style={{ ...tdStyle, fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{p.stop?.substring(0, 12)}</td>
          ...
          {tab === "bulk" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
              <div style={{ maxWidth: 600 }}>
                <div style={{ color: "#00c8ff", fontSize: 13, fontWeight: 700, letterSpacing: 2, marginBottom: 20, textTransform: "uppercase" }}>Búsqueda y Sustitución Masiva</div>
                <div style={{ background: "rgba(0,200,255,0.03)", border: "1px solid rgba(0,200,255,0.1)", borderRadius: 2, padding: 20, marginBottom: 20 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                    <div>
                      <label style={labelStyle}>CAMPO</label>
                      <select value={bulkField} onChange={e => setBulkField(e.target.value)} style={{ ...selectStyle, width: "100%" }}>
                        <option value="name">Nombre</option>
                        <option value="group">Grupo</option>
                        <option value="tvgId">TVG-ID</option>
                        <option value="tvgName">TVG-Name</option>
                        <option value="logo">Logo URL</option>
                        <option value="url">URL Stream</option>
                      </select>
                    </div>
                  </div>
                  <div style={{ marginBottom: 12 }}>
                    <label style={labelStyle}>BUSCAR</label>
                    <input value={bulkSearch} onChange={e => setBulkSearch(e.target.value)} placeholder="Texto a buscar..." style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />
                  </div>
                  <div style={{ marginBottom: 16 }}>
                    <label style={labelStyle}>REEMPLAZAR CON</label>
                    <input value={bulkReplace} onChange={e => setBulkReplace(e.target.value)} placeholder="Texto de reemplazo..." style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />
                  </div>
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    <button onClick={applyBulkReplace} style={btnStyle("#00ff88")}>✓ Aplicar a {playlist?.channels.length || 0} canales</button>
                    {bulkSearch && (
                      <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>
                        {playlist?.channels.filter(c => c[bulkField]?.includes(bulkSearch)).length || 0} coincidencias en "{bulkField}"
                      </span>
                    )}
                  </div>
                </div>
                <div style={{ color: "rgba(0,200,255,0.6)", fontSize: 10, letterSpacing: 2, marginBottom: 12 }}>ESTADÍSTICAS DE PLAYLIST</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                  {[
                    ["Total Canales", playlist?.channels.length || 0],
                    ["Grupos", groups.length],
                    ["Con Logo", playlist?.channels.filter(c => c.logo).length || 0],
                    ["Con TVG-ID", playlist?.channels.filter(c => c.tvgId).length || 0],
                    ["Sin URL", playlist?.channels.filter(c => !c.url).length || 0],
                    ["Seleccionados", selectedChannels.size],
                  ].map(([label, val]) => (
                    <div key={label} style={{ background: "rgba(0,200,255,0.04)", border: "1px solid rgba(0,200,255,0.08)", padding: "12px 16px", borderRadius: 2 }}>
                      <div style={{ color: "#00c8ff", fontSize: 20, fontWeight: 700 }}>{val}</div>
      ...
      {editingChannel && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#0d1622", border: "1px solid rgba(0,200,255,0.3)", borderRadius: 4, padding: 28, width: 480, maxWidth: "95vw" }}>
            <div style={{ color: "#00c8ff", fontWeight: 700, letterSpacing: 2, marginBottom: 20, textTransform: "uppercase" }}>Editar Canal</div>
            {[
              ["NOMBRE", "name"], ["GRUPO", "group"], ["TVG-ID", "tvgId"],
              ["TVG-NAME", "tvgName"], ["LOGO URL", "logo"], ["STREAM URL", "url"]
            ].map(([lbl, field]) => (
              <div key={field} style={{ marginBottom: 12 }}>
                <label style={labelStyle}>{lbl}</label>
                <input value={editingChannel[field] || ""} onChange={e => setEditingChannel({ ...editingChannel, [field]: e.target.value })} style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />
              </div>
            ))}
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button onClick={() => saveChannelEdit(editingChannel)} style={btnStyle("#00ff88")}>✓ Guardar</button>
              <button onClick={() => setEditingChannel(null)} style={btnStyle("rgba(255,255,255,0.2)")}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", bottom: 24, right: 24, background: toast.type === "success" ? "rgba(0,255,136,0.1)" : "rgba(255,68,85,0.1)", border: `1px solid ${toast.type === "success" ? "rgba(0,255,136,0.4)" : "rgba(255,68,85,0.4)"}`, color: toast.type === "success" ? "#00ff88" : "#ff4455", padding: "12px 20px", borderRadius: 2, fontSize: 13, zIndex: 2000, backdropFilter: "blur(10px)" }}>
          {toast.msg}
        </div>
      )}
    </div>
